...
/// 行为准则（Plan25 #1 新增）：把「怎么做事」的不可变工作风格沉淀为一段，
/// 与上面「能用什么工具」（纪律）互补。原则精炼、可执行，不与工具纪律重复罗列工具名。
...
5）写完必验证：凡改了代码，收尾前主动跑可用的构建 / 测试 / 诊断（如 cargo check、npm test、.mdga/diagnostics）确认没引入错误；有错就修到通过或讲清为何放弃。\
6）不可逆操作要谨慎：删除目录、批量删文件、git 重置 / 强推等不可回退操作执行前先确认必要性，宁可先征询也不要贸然。\
