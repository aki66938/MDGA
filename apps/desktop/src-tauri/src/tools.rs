    ...
    vec![
        ...
        serde_json::json!({
            ...
            "function": {
                ...
                "description": "Render an INTERACTIVE artifact inline in the conversation — agent-authored, fully self-contained HTML/SVG/JS that runs live in a sandboxed canvas the user can click, type into, and play with. Reach for this whenever a LIVE, interactive thing beats a static answer: calculators, interactive forms, clickable/zoomable charts & dashboards, simulations, step-through demos, animated diagrams, games, before/after sliders, mini-tools. PREFER INTERACTION & MOTION over static pictures — a static SVG is the fallback, not the goal; if the topic can react to the user, make it react.\n\n★ BUILT-IN UI RUNTIME (already loaded in the sandbox — NO imports, NO external libs needed). Its components are interactive BY CONSTRUCTION, so PREFER it for any controls / forms / dashboards / product-design mockups — you don't have to get the interactivity JS right, this is the most reliable way to produce working interactive UI:\n- Globals: h(tag, props, ...children) [props: class, style (object or string), text, value, checked, on<Event> handlers, any attribute]; UI.mount(viewFn) [renders viewFn() into <body>, re-renders after store.set]; UI.store(init) -> {get(key), set(patch), state}; UI.mountSpec(spec).\n- Components (EACH is interactive on its own even with NO state-wiring — sliders drag, switches flip, tabs switch, inputs type; all auto-theme to host light/dark): UI.Button({label,onClick,variant:'primary'|'default'|'ghost'|'danger'}), UI.Input({value,placeholder,type,onInput}), UI.Textarea({value,rows,onInput}), UI.Select({value,options:[{label,value}],onChange}), UI.Checkbox({label,checked,onChange}), UI.Switch({label,checked,onChange}), UI.Slider({min,max,value,step,label,onInput}), UI.Tabs({tabs:[{label,content}],active}), UI.Card({title},...children), UI.Field({label,hint,control}), UI.Badge({text,tone:'brand'|'success'|'danger'}), UI.Heading({text,level}), UI.Text({text,muted}), UI.Progress({value,max}), UI.Divider(), UI.Stack({gap},...children), UI.Row({gap},...children), UI.Col(...children).\n- Counter example: <div id=app></div><script>var s=UI.store({n:0});UI.mount(function(){return UI.Row(UI.Button({label:'-1',onClick:function(){s.set({n:s.get('n')-1})}}),UI.Text({text:'count = '+s.get('n')}),UI.Button({label:'+1',variant:'primary',onClick:function(){s.set({n:s.get('n')+1})}}));},document.getElementById('app'));</script>\n\n★ DECLARATIVE MODE — the MOST reliable option for forms / settings / dashboards / standard control layouts: set `code` to a JSON object describing a node tree (no code at all); the runtime renders it so it ALWAYS works. A node is {\"type\":<name>, ...props, \"children\":[...]}; type is one of button|input|textarea|select|checkbox|switch|slider|tabs|card|field|badge|heading|text|progress|divider|stack|row|col. A top-level {\"title\":\"...\"} adds a heading. Example (sign-up form): {\"title\":\"Sign up\",\"children\":[{\"type\":\"field\",\"label\":\"Name\",\"control\":{\"type\":\"input\",\"placeholder\":\"Your name\"}},{\"type\":\"field\",\"label\":\"Age\",\"control\":{\"type\":\"slider\",\"min\":0,\"max\":100,\"value\":30}},{\"type\":\"switch\",\"label\":\"Subscribe to updates\"},{\"type\":\"button\",\"label\":\"Create account\",\"variant\":\"primary\"}]}\n\nWHEN TO USE WHICH — decide by whether the user wants to SEE AN EFFECT when they interact:\n- Just a LAYOUT MOCKUP (show what a UI looks like; controls move/toggle on their own but DON'T need to drive each other) → DECLARATIVE JSON spec. Note: declarative renders controls but wires NO cross-control effect — clicking a toggle will NOT change anything else. Use it ONLY when a static look is the goal.\n- WORKING interactions where touching one thing visibly changes another ('a dark-mode toggle that recolors the panel', a calculator, a color picker with a live swatch, 'when I click/drag X, Y updates') → raw HTML/JS with the UI runtime. Do NOT use declarative for these. Two patterns:\n  (1) CLICK / state change → UI.store + UI.mount (re-renders on store.set). Dark-mode example: <div id=app></div><script>var s=UI.store({dark:false});UI.mount(function(){var d=s.get('dark');return UI.Card({},UI.Switch({label:'Dark mode',checked:d,onChange:function(v){s.set({dark:v})}}),h('div',{style:'margin-top:10px;padding:16px;border-radius:8px;background:'+(d?'#1c2128':'#fff')+';color:'+(d?'#e6edf3':'#111')+';'},'This panel recolors live.'));},document.getElementById('app'));</script>\n  (2) SLIDER / INPUT → live preview → do NOT use store (its re-render interrupts dragging); keep a reference to the preview element and update it directly inside onInput. Color-picker example: <script>var pv=h('div',{style:'width:80px;height:80px;border-radius:8px;border:1px solid var(--border);'});var c={r:128,g:128,b:128};function paint(){pv.style.background='rgb('+c.r+','+c.g+','+c.b+')';}paint();function mk(k){return UI.Slider({label:k.toUpperCase(),min:0,max:255,value:c[k],onInput:function(v){c[k]=v;paint();}});}document.body.appendChild(UI.Stack({},mk('r'),mk('g'),mk('b'),pv));</script>\n- A plain static <svg> is the last-resort fallback.\n\nDO:\n- Make it INTERACTIVE: wire up real behavior with addEventListener / onclick / oninput etc. (inline event handlers in your <script>). Respond to clicks, input, hover, drag.\n- Make it ALIVE: animate with native CSS transitions/animations/@keyframes and/or plain requestAnimationFrame — transitions on state change, entrance animations, animated charts, etc.\n- The canvas is FULLY ISOLATED — it has NO channel back to the conversation and CANNOT navigate the browser. There is NO `sendPrompt`, NO `openLink`, no host bridge of any kind. Buttons, forms, and 'Submit'/'Save' controls may run for real, but their effect stays ENTIRELY inside the card: update the artifact's own state and show feedback in-card (e.g. a 'Save' button shows '✅ saved' right there). Nothing the user types or clicks in the card is ever sent to you or anywhere else. Do NOT write code that tries to message the chat or open URLs — those globals don't exist. If you need something back from the user, just ask in your normal reply.\n- Keep it SELF-CONTAINED: write all data and any helper code directly inline. Use the CSS variables the host injects so it matches the app's light/dark theme (e.g. var(--color-text), var(--color-bg), var(--color-text-secondary), var(--brand), var(--border)).\n\nDON'T (these are blocked by the sandbox and will silently fail — do not even try):\n- NO `eval` or `new Function(...)` (Content-Security-Policy has no unsafe-eval) — write real code, never build it from strings.\n- NO external resources of ANY kind — no CDN, no `<script src>`, no `<link>` stylesheet, no external fonts/images/icons. You do NOT need any external UI library — the UI runtime is already built in; inline every script and style; embed images as data: URIs.\n- NO network: no `fetch`, no XMLHttpRequest, no WebSocket, no EventSource (connect-src is none). You cannot load or fetch anything at runtime — inline the data instead.\n- NO storage / ambient identity: no localStorage, sessionStorage, cookies, IndexedDB, or document.domain (the artifact runs from a null origin).\n\nThe artifact runs in a STRICT, isolated sandbox (sandboxed iframe, null origin, zero-network CSP, no filesystem, no access to the app or its data). These limits are a deliberate security boundary and CANNOT be worked around, so design within them: bake every bit of data and logic inline into the single artifact. `code` is EITHER (a) a JSON spec object (declarative mode, starts with `{`), OR (b) raw SVG (must start with `<svg`), OR (c) an HTML body fragment (NO doctype/html/head/body wrapper — just body content; inline <style> and <script> are allowed, and the built-in UI runtime `UI`/`h` is pre-loaded).",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "code": { "type": "string", "description": "The artifact source, ONE of: (a) a JSON spec object for declarative mode (starts with `{` — a node tree of standard controls, MOST reliable for forms/dashboards), (b) raw SVG (starts with <svg), or (c) an HTML body fragment (no doctype/html/head wrapper). For (c), inline <style>/<script> are allowed and the built-in UI runtime (`UI`/`h`, interactive components) is pre-loaded — prefer it for controls; add motion via CSS/requestAnimationFrame. All resources inline or data: URIs — no external/CDN refs, no eval, no network. Max 512KB." },
        ...
        serde_json::json!({
            ...
            "function": {
                "name": "run_subtask",
                "description": "Delegate a focused subtask to a sub-agent with its own fresh context, returning a concise text report. BY DEFAULT the sub-agent is fully capable — it can read, edit files, AND run commands — under the SAME permission gating and checkpoints as the main agent (every write/delete/command is user-approved and checkpointed). So you can just delegate real work (e.g. 'detect the toolchain and report versions', 'implement X') without any special flag. Set mode='read' to restrict it to SAFE READ-ONLY investigation (list/read/search only, sandboxed, no network) — use this for pure exploration like 'find where X is implemented'. Use this to keep large investigations or self-contained work off the main conversation. Optionally set agentType to use a custom agent role from .mdga/agents/<type>.md. background=true runs asynchronously (returns a taskId; poll with get_task_output, stop with kill_task); since an async task can't get interactive approvals, background implies read-only (it's for read-only exploration only — for command/edit work, run it in the foreground).",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "description": { "type": "string", "description": "Clear, self-contained description of what to do and what the report should contain." },
                        "mode": { "type": "string", "enum": ["read", "write"], "description": "Omit (default) = CAPABLE: may read, edit files, and run commands under the main agent's permission gating + checkpoints. 'read' = restrict to safe read-only exploration (sandboxed, no writes/commands/network). 'write' is accepted as an explicit synonym of the default capable mode." },
                        "agentType": { "type": "string", "description": "Optional custom agent type name (loads .mdga/agents/<type>.md as the sub-agent role)." },
                        "background": { "type": "boolean", "description": "Run asynchronously: return a taskId immediately instead of blocking. Poll with get_task_output, stop with kill_task. Implies read-only (an async task can't get approvals); a capable/'write' subtask always runs in the foreground, so background is ignored for it. Default false." }
                    },
...
fn execute_render_artifact(arguments: &str) -> Result<serde_json::Value, String> {
    ...
    // 0.0.80：类型判定加「声明式 spec」一档。code 去空白后以 `{` 开头且能解析成 JSON 对象 → spec
    //（由前端 UI.mountSpec 渲染,弱模型可控产出可交互稿）。否则 svg / html 同旧。
    let trimmed = args.code.trim_start();
    let is_spec = trimmed.starts_with('{')
        && serde_json::from_str::<serde_json::Value>(args.code.trim())
            .map(|v| v.is_object())
            .unwrap_or(false);
    let kind = if is_spec {
        "spec"
    ...
    // warning（=自修复信号），让 agent 在同一轮改用内置 UI 运行时/内联实现重写。不改渲染、不改安全模型、不报错。
    // spec 是**数据**不是代码 → 不扫（避免 JSON 里的字符串误触发）。
            ...
            obj.insert(
                ...
                serde_json::Value::String(format!(
                    "互动卡片在断网的沙箱（严格 CSP、null origin）里渲染。启发式扫描检测到可能的外部资源/动态求值用法——{}（按子串匹配，若仅出现在文本/注释中可忽略）：\
                     若用于真正加载外部资源或动态求值，会被静默拦截并 no-op、卡片会「坏掉」。请**在本轮直接重写**:\
                     不要引任何外部库/CDN/eval——你已经有**内置 UI 运行时**（全局 `UI`/`h` 组件,自带交互、自动主题,无需外部库）；\
                     或对以标准控件为主的设计稿,直接把 `code` 改成一段**声明式 JSON spec**（{{type,...,children}}）交宿主可靠渲染。",
                    hits.join("、")
                )),
            );
        }
