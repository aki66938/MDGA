...
3. **`<ToolCall>` XML 风格变体**（0.0.17 dev 实测）：模型有时既不走结构化 tool_calls 也不用 DSML，而是输出 `<ToolCall name="search_file"> <parameter name="pattern" string="true">...</parameter> </ToolCall>` 这种 XML 风格标记，且**工具名可能是幻觉**（如不存在的 search_file）。
   - 解法：解析器泛化为 `parse_markup_tool_calls()`（标记可配置），先试 DSML 变体、再试 `<ToolCall>`/`<parameter>` 变体；`strip_dsml_markup()` 同时在 `<DSML` 与 `<ToolCall` 处截断。幻觉工具名交给执行层报「未知工具」+ 纠错 hint，让模型自行换用真实工具。
