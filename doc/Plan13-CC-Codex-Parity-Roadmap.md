...
> **待做队列**（主创确认的优先级）：
> - **近期**：MCP SSE/HTTP 传输（0.0.25 单独做）✅、MCP 完整 OAuth ✅、并行子代理执行 → **已并入 Plan15 M12.3**。
> - **下一阶段**：经新一轮三方对照，结构化澄清提问 / 异步后台代理 / worktree 并行可写子代理 / 工具面补强（Grep 富参数·Glob·MCP Resources·大输出持久化）整理为 **Plan15（M12）**。
> - **放缓（主创定）**：M8.2 AppContainer（待 M8.1 受限令牌沙箱真机验证后再启动）。
> - **放缓（主创定）**：视觉/图片、思考预算、outputStyle / statusLine 自定义。
