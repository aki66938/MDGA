...
| R3 | **真 TDD 自修复循环** | 现诊断只是"跑配置命令喂回错误"~2 轮,非"跑测试→读失败→打补丁到绿";自修复是 2026 最大分水岭 | M:结构化解析测试结果 + 迭代到绿 + doom-loop 护栏 + 可选按影响选测,长在现有诊断循环上 | `feat/r3-tdd` · 本会话 2026-06-18 实现(Phase1+2)· 待回归/合并 |
| R4 | **Git 原生工具** | git 现只能 run_command 裸跑字符串,无结构化 commit/diff/branch/PR;git 是可审计/可回滚/可交付底座 | M:git2/gh 包成工具,复用权限/审计层 | `feat/r4-git` · 进行中 |
| R5 | **跨文件原子编辑** | apply_patch 明确仅单文件;改函数名+全部调用点没法一次原子完成,大重构脆弱 | M:扩成多文件 patch set(all-or-nothing + revert),配合 R1 重命名 | 未认领 |

> **R3 实现说明(feat/r3-tdd,Phase1+2 已落地)**
> - 新增纯逻辑模块 `crates/agent-core/src/test_report.rs`(零正则零外部依赖,18 单测):
>   `detect_verification_plan`(有序「编译门→测试门」计划)、`parse_report`(cargo test/cargo check/jest·vitest/pytest/go test 结构化解析,逐失败名+file:line+消息)、`format_verify_feedback`(结构化回灌,解析不出回退原始尾部)、`report_signature`(doom-loop 失败签名)、`focused_command`(按影响选测)。
> - 改写 `agent_loop.rs` 验证回路:两段门顺序跑、首个失败门即结构化回灌;`VERIFY_MAX_ROUNDS` 2→5,新增 `VERIFY_STALL_THRESHOLD=2` doom-loop 护栏(失败集合连续不变即 emit `agent-stuck` 升级用户);测试门失败用例下轮窄跑、窄跑转绿后复跑整套确认。
> - **`.mdga/diagnostics` 新语法**(向后兼容裸命令行):多行 `build: <cmd>` / `test: <cmd>` 显式声明两段门;裸行仍按旧语义单步(含 test 视为测试门)。
> - 验证:`cargo test -p mdga-agent-core`(18 绿)+ `cargo test -p mdga-desktop --bins`(15 绿,含 2 条护栏单测)。**未碰 tools.rs/main.rs,无 R1/R4/R5 合并热点冲突。**
