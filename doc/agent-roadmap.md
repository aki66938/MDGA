...
| R10 | 并行可写子代理(worktree 隔离) | 写子代理共享同工作区、无隔离,做不了安全 fan-out | L:Windows worktree/写时复制 + 合并冲突处理,交叉 AppContainer 工作 | 🔵 **波2 进行中**(feat/r10-worktree)——风险最高,保守实现:worktree 隔离写子代理原语 + 显式合并回主;做不安全则只交原语+接入点,不自动开启 |

### 🟢 低优先

| ID | 缺口 | 量级 | 认领状态 |
|----|------|------|---------|
| R11 | 自动生成 repo wiki(可查询代码库知识) | L(依赖 R2 先做) | 🔵 **波2 进行中**(feat/r11-wiki)——基于 codemap 生成各模块摘要落 `.mdga/wiki/` + 查询工具 |
