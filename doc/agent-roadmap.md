...
| R2 | **repo/codemap → 语义检索** | 现只有文件树摘要 + code_overview,无法按语义找代码、无引用排名定位 | 先 M(tree-sitter+PageRank repo map,免基础设施、最划算第一步),后 L(向量索引 chunk/embed/store/sync) | `feat/r2-codemap` · 本会话(2026-06-18)· **M 阶段已实现**:新 crate `mdga-codemap`(tree-sitter 抽取定义/引用 + 个性化 PageRank,rs/py/js/ts/tsx/go)→ `repo_map` 只读工具(focus/query/maxTokens)+ 开局自动注入符号地图;单测 6 + 全量回归绿,待合并。**L 阶段(向量检索)未启** |
