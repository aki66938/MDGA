...
| ID | 缺口 | 为什么关键 | 量级 | 认领状态 |
|----|------|-----------|------|---------|
| R1 | **LSP 集成**(跳转/引用/类型/即时诊断) | 现零 LSP,导航靠文本搜、编辑靠字节匹配、无符号感知 → 大/强类型库臆造符号、静默回归。补上=编译级真相 | L:新 crate 包 rust-analyzer/tsserver/pyright over JSON-RPC,暴露 find_symbol/find_references/hover/diagnostics 工具,复用现成进程/ConPTY | ❌ **未实现**(feat/r1-lsp 仅在路线图占坑、零代码,经验收确认)——需重做 |
| R2 | **repo/codemap → 语义检索** | 现只有文件树摘要 + code_overview,无法按语义找代码、无引用排名定位 | 先 M(tree-sitter+PageRank repo map,免基础设施、最划算第一步),后 L(向量索引 chunk/embed/store/sync) | ✅ **M 阶段已验收合并(0.0.52)**:新 crate `mdga-codemap`(tree-sitter 抽取定义/引用 + 个性化 PageRank)→ `repo_map` 只读工具 + 开局自动注入符号地图;纯内存、无联网、有内存上限。技术债:首次建图持锁同步全仓 parse(待挪 spawn_blocking)、tags 缓存无淘汰。**L 阶段(向量检索)未启** |
| R3 | **真 TDD 自修复循环** | 现诊断只是"跑配置命令喂回错误"~2 轮,非"跑测试→读失败→打补丁到绿";自修复是 2026 最大分水岭 | M:结构化解析测试结果 + 迭代到绿 + doom-loop 护栏 + 可选按影响选测,长在现有诊断循环上 | ⛔ **已实现但验收挡下**(feat/r3-tdd 保留):思路/解析好,但有 2 个安全阻断——① 验证循环硬编码 `sandbox=false` 裸跑 cargo test/pytest/build(违背用户沙箱设置、放大执行任意项目代码);② focused_command 把解析自测试输出的失败名未转义拼进 `powershell -Command`(命令注入/RCE)。待修这俩再合 |
