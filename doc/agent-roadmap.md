...
| R4 | **Git 原生工具** | git 现只能 run_command 裸跑字符串,无结构化 commit/diff/branch/PR;git 是可审计/可回滚/可交付底座 | M:git2/gh 包成工具,复用权限/审计层 | `feat/r4-git` · 本会话 · **本地 v1 已实现**(壳调 git CLI:status/diff/log/branch/add/commit,只读→FileRead 自动放行、写→FileWrite;含单测+端到端冒烟,待 review/merge);push+PR(gh)留后续增量 |
