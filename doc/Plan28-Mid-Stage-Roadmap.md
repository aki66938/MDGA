...
- **P3-10 AppContainer 沙箱(M8.2)——✅ 已接通为默认(0.0.40)**:文件路径 + 网络隔离,补齐安全纵深。
  - **0.0.39(基座)**:`appcontainer_win.rs` 实现并真机验证文件/网络隔离,但门控未启用——遗留拦路问题:
    容器内 native console 程序(git/npm/node…)stdout/stderr **不回传**(AppContainer 断开「容器进程 →
    其子进程」的 I/O 继承);当时连 ConPTY 限时 spike 都判为无解。
  - **0.0.40(攻克并接通)**:顺「精简镜像」线索深挖后定位到这其实是 AppContainer 固有设计,且有微软文档化
    正规解。补齐四件套:① `HKCU\Console\LowBoxConsoleEnabled=1` + ② `lpacCom`/`registryRead` 能力 +
    ③ ConPTY 伪控制台(子进程 NULL std 句柄强制走 PTY)+ ④ `New-PSDrive` 把工作区挂成 PSDrive 进入
    (规避 Set-Location 到用户 profile 的「拒绝访问」),native 命令输出完整回传、cwd 与相对路径落在工作区。
  ...
  - **跨版本健壮性 + 去注册表(0.0.40 → 0.0.41)**:`appcontainer_self_test()` 每进程实跑探针(nonce 防误判)验证
